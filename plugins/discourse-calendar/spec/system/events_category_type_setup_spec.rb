...
RSpec.describe "Events Category Type Setup" do
  ...
  context "when visiting the Events tab for a non-events category" do
    fab!(:category)

    it "shows the not events type message" do
      visit("/c/#{category.slug}/edit/events")
      expect(page).to have_content(
        I18n.t("js.discourse_calendar.category_type_events.not_events_type"),
      )
    end
  end
end
